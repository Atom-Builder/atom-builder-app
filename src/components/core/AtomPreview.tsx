...
// import { useBuilder } from '@/hooks/useBuilder'; // <-- REMOVE THIS - Not needed here

// ... (Electron, Nucleus, OrbitLine components remain the same) ...
...
// --- FIX Starts Here ---
// AtomScene now accepts props instead of using useBuilder()
interface AtomSceneProps {
    protons: number;
    neutrons: number;
    electrons: number;
    isAntimatter: boolean;
    // vizMode is not needed for preview as it always uses Bohr
}
...
// --- FIX: Define props explicitly here too ---
    ...
    return (
        <Canvas camera={{ position: [0, 5, 10], fov: 50 }}>
            ...
            <Suspense fallback={null}>
                {/* --- FIX: Pass props down to AtomScene --- */}
                <AtomScene
                    protons={protons}
                    neutrons={neutrons}
                    electrons={electrons}
