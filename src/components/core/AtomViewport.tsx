        ...
        <group ref={groupRef}>
            {particles.map((p, i) => (
                <Sphere key={i} args={[particleRadius, PARTICLE_SEGMENTS, PARTICLE_SEGMENTS]} position={p.position as [number, number, number]}>
                    <meshPhysicalMaterial color={p.color} emissive={p.color} emissiveIntensity={0.2} roughness={0.6} metalness={0.1} clearcoat={0.1}/>
                </Sphere>
            ))}
        </group>
    );
...
function OrbitLine({ radius, electronColor }: OrbitLineProps) {
    const lineObject = useMemo(() => { /* ... <primitive> logic ... */
     ...
     React.useEffect(() => { return () => { lineObject.geometry.dispose(); lineObject.material.dispose(); } }, [lineObject]);
    return <primitive object={lineObject} dispose={null} />;
}

// ... (AtomScene and AtomViewport components remain the same) ...
...
function AtomScene({ protons, neutrons, electrons, isAntimatter, vizMode }: AtomSceneProps) {
    ...
    return (
        <Suspense fallback={null}> /* ... Scene content ... */
